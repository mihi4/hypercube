...
module frame_mount()
{
	...
	difference()
	{
		...
		for (a = hole_offsets)
			translate(a)
			{
                cylinder(r = 2.75, h = height * 2 + 2 * epsilon, center = true, $fn = 16); // Mounting holes for 2020 extrusions, michi
				//cylinder(r = m3_wide_radius+1, h = height * 2 + 2 * epsilon, center = true, $fn = 16);
                
				//cylinder(r = m3_head_radius+1, h = height + epsilon, $fn = 16);
			}
...
module filament_tunnel()
{
	...
	translate([0, 0, height / 2])
	{
		difference()
		{
			...
			translate([-height - width / 2 -7, 0, height / 2]) //change michi
				rotate([90, 0, 0])
					//cylinder(r = height, h = length + 2 * epsilon, center = true, $fn = 32);
					cylinder(r = height+2, h = length + 2 * epsilon, center = true, $fn = 32); // change michi
